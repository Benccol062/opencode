...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: "在差异查看器中下移" },
      { key: "k,up", cmd: "diff.up", desc: "在差异查看器中上移" },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: "在差异查看器中向下翻页" },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: "在差异查看器中向上翻页" },
      { key: "m", cmd: "diff.mark_reviewed", desc: "将所选文件标记为已审阅" },
      ...
      <PanelGroup axis="y" width="100%" height="100%">
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          ...
          <Show when={nextFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>下一个文件</span>
              </text>
            )}
          </Show>
          <Show when={nextHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>下一个差异块</span>
              </text>
            )}
          </Show>
          <Show when={previousHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>上一个差异块</span>
              </text>
            )}
          </Show>
          <Show when={previousFileShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>上一个文件</span>
              </text>
            )}
          </Show>
          <Show when={switchSourceShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>切换数据源</span>
              </text>
            )}
          </Show>
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>标记已审阅</span>
              </text>
            )}
          </Show>
...
function DiffViewerHelpDialog() {
  ...
  const rows = [
    ...
    },
    {
      ...
      action: "下一个文件",
      description: "按文件树顺序选择下一个已更改的文件",
    },
    {
      shortcut: useCommandShortcut("diff.previous_file"),
      action: "上一个文件",
      description: "按文件树顺序选择上一个已更改的文件",
    },
    {
    ...
    },
    {
      ...
      action: "切换补丁",
      description: "在单个选定补丁和所有补丁之间切换",
    },
    {
      shortcut: useCommandShortcut("diff.switch_source"),
      action: "切换数据源",
      description: "选择工作区、主分支或上一轮的更改",
    },
    {
  ...
  return (
    <box paddingLeft={2} paddingRight={2} paddingBottom={1} gap={1}>
      ...
      <box flexDirection="row">
        ...
        <text fg={theme.textMuted}>说明</text>
      </box>
      <For each={rows}>
        {(row) => (
          <box flexDirection="row">
